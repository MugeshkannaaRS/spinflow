...
function PlanFormDialog({ plan, onClose }: { plan?: Plan | null; onClose: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    module_prices: (plan?.module_prices ?? MODULE_NAMES.map((m) => ({
      module_name: m, monthly_price: 999, yearly_price: 9990, is_included: false,
    }))).map((mp) => ({
  ...
  return (
    <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
      ...
      <div className="grid grid-cols-2 gap-4 py-4">
        ...
        <div>
          <Label>Code</Label>
          <Input value={form.code} onChange={(e) => setForm((f) => ({ ...f, code: e.target.value }))} disabled={!!plan} />
        </div>
        <div>
        ...
        <div>
          ...
          <Input type="number" value={form.included_mills} onChange={(e) => setForm((f) => ({ ...f, included_mills: Number(e.target.value) }))} />
        </div>
        <div>
          <Label>Included Users</Label>
          <Input type="number" value={form.included_users} onChange={(e) => setForm((f) => ({ ...f, included_users: Number(e.target.value) }))} />
        </div>
        <div>
          <Label>Additional Mill Cost (₹/mo)</Label>
          <Input type="number" value={form.additional_mill_cost} onChange={(e) => setForm((f) => ({ ...f, additional_mill_cost: Number(e.target.value) }))} />
        </div>
        <div>
          <Label>Additional User Cost (₹/mo)</Label>
          <Input type="number" value={form.additional_user_cost} onChange={(e) => setForm((f) => ({ ...f, additional_user_cost: Number(e.target.value) }))} />
        </div>
      </div>
      ...
      <div className="border rounded-lg p-4">
        ...
        <div className="space-y-2">
          {form.module_prices.map((mp) => (
            <div key={mp.module_name} className="flex items-center justify-between gap-4 py-1.5 border-b last:border-0">
              <div className="flex items-center gap-3">
                <Switch
                  checked={mp.is_included}
                  onCheckedChange={(v) => updateModule(mp.module_name, "is_included", v)}
                />
                <span className="text-sm font-medium w-24">{MODULE_LABELS[mp.module_name] ?? mp.module_name}</span>
              </div>
              {!mp.is_included && (
                <div className="flex gap-2">
                  <div className="flex items-center gap-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => updateModule(mp.module_name, "monthly_price", Number(e.target.value))}
                    />
                    <span className="text-xs text-muted-foreground">/mo</span>
                  </div>
                </div>
              )}
              {mp.is_included && <span className="text-xs text-green-600 font-medium">Included</span>}
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={() => { setEditPlan(null); setDialogOpen(true); }}>
              ...
              <Card key={plan.id} className="relative">
                <CardContent className="p-5">
                  <div className="flex items-start justify-between">
                    ...
                    <Button variant="ghost" size="icon" className="size-8" onClick={() => { setEditPlan(plan); setDialogOpen(true); }}>
                      <Pencil className="size-4" />
                    </Button>
                  </div>
                  <div className="mt-4 space-y-1">
                    <div className="flex justify-between text-sm"><span>Monthly</span><span className="font-semibold">₹{plan.monthly_price?.toLocaleString("en-IN")}</span></div>
                    <div className="flex justify-between text-sm"><span>Yearly</span><span className="font-semibold">₹{plan.yearly_price?.toLocaleString("en-IN")}</span></div>
                    <div className="border-t my-2" />
                    <div className="flex justify-between text-xs text-muted-foreground"><span>Included Mills</span><span>{plan.included_mills}</span></div>
                    <div className="flex justify-between text-xs text-muted-foreground"><span>Included Users</span><span>{plan.included_users}</span></div>
                    <div className="flex justify-between text-xs text-muted-foreground"><span>Extra Mill Cost</span><span>₹{plan.additional_mill_cost?.toLocaleString("en-IN")}/mo</span></div>
                    <div className="flex justify-between text-xs text-muted-foreground"><span>Modules</span><span>{included} included + {addon} addon</span></div>
                  </div>
                </CardContent>
              </Card>
            );
      ...
      <Dialog open={dialogOpen} onOpenChange={(o) => { setDialogOpen(o); if (!o) setEditPlan(null); }}>
        {dialogOpen && <PlanFormDialog plan={editPlan} onClose={() => { setDialogOpen(false); setEditPlan(null); }} />}
      </Dialog>
    </div>
  );
}
