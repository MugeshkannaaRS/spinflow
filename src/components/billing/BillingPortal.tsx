...
const MODULE_ICONS: Record<string, React.ElementType> = {
  ...
  masters: Settings2,
  users: UserCog,
  column_config: SlidersHorizontal,
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm mx-4 p-6">
        ...
        <div className="flex justify-end gap-2">
          <button
            onClick={onCancel}
            className="px-4 py-2 rounded-lg border border-[#d1d5db] text-[13px] font-medium text-[#374151] hover:bg-gray-50"
          >
            Cancel
          </button>
          <button
          ...
          <div className="absolute top-full left-0 mt-1 z-20 bg-white border border-[#e2e8f0] rounded-lg shadow-lg p-2 min-w-40 max-h-48 overflow-y-auto">
            {modules.length === 0 ? (
              ...
              modules.map((m) => (
                <div
                  key={m}
                  className="flex items-center gap-1.5 px-2 py-1 text-[12px] text-[#374151]"
                >
            ...
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={ov.revenue_trend}>
                ...
                <YAxis
                  ...
                  axisLine={false}
                  tickLine={false}
                  tickFormatter={(v: number) => fmtLakh(v)}
                />
                <Tooltip
                  contentStyle={{
                    background: "#fff",
                ...
                <thead>
                  <tr className="bg-[#f1f5f9] border-b border-[#e2e8f0]">
                    {[
                      ...
                      <th
                        key={h}
                        className="text-left px-4 py-3 text-[#475569] font-semibold text-xs uppercase tracking-wide whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {companiesQ.isLoading ? (
                    ...
                    comps.map((co: any) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <ModulesCell modules={co.enabled_modules} />
                        </td>
                        <td className="px-4 py-3 font-mono text-[#0f172a]">
                          {fmtLakh(co.monthly_amount)}
                        </td>
                        <td className="px-4 py-3 relative">
                          <div className="flex items-center gap-1 flex-wrap">
                            ...
                            <button
                              onClick={() =>
                                setModulesPopover(modulesPopover?.id === co.id ? null : co)
                              }
                              ...
                              <div className="absolute right-0 top-full mt-1 z-30 bg-white border border-[#e2e8f0] rounded-xl shadow-lg p-3 min-w-48 max-h-52 overflow-y-auto">
                                ...
                                {(co.enabled_modules ?? []).length === 0 ? (
                                  ...
                                  co.enabled_modules.map((m: string) => (
                                    <div
                                      key={m}
                                      className="flex items-center gap-1.5 py-0.5 text-[12px] text-[#374151]"
                                    >
                                      <span className="w-1.5 h-1.5 rounded-full bg-green-500 inline-block" />
                                      {m
                                        .replace(/_/g, " ")
                                ...
                                <a
                                  href="/admin/modules"
                                  className="block mt-2 text-[11px] text-blue-600 hover:underline border-t border-[#f1f5f9] pt-1.5"
                                >
                                  Edit in Admin Panel →
                                </a>
                              </div>
            ...
            <div className="px-5 py-3 border-t border-[#e2e8f0] flex items-center justify-between text-[13px] text-[#64748b]">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1 rounded border border-[#d1d5db] hover:bg-gray-50 disabled:opacity-40"
                >
                  Prev
                </button>
...
function OveragePurchaseDialog({
  ...
  const qc = useQueryClient();
  const [resource, setResource] = useState<"extra_users" | "extra_mills" | "extra_employees">(
    "extra_users",
  );
  const [qty, setQty] = useState(1);

  const unitPrice =
    resource === "extra_users"
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
        ...
        <div className="px-6 py-4 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {(["extra_users", "extra_mills", "extra_employees"] as const).map((r) => (
                <button
                  ...
                  className={cn(
                    "px-3 py-2 rounded-lg border text-[12px] font-medium text-center transition-all",
                    resource === r
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-8 h-8 rounded-lg border border-[#d1d5db] flex items-center justify-center text-lg font-medium hover:bg-gray-50"
              >
                −
              </button>
              <input
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            ...
            <p className="text-[24px] font-bold font-mono text-[#0f172a]">
        ...
        <div className="px-6 py-4 border-t border-[#e2e8f0] flex justify-end gap-2">
          ...
          <button
            onClick={() => purchaseMut.mutate({ resource_type: resource, quantity: qty })}
...
function UpgradeDialog({
  ...
  const upgradePlans = (Array.isArray(plansQ.data) ? plansQ.data : [])
    .filter(
      (p: any) =>
        p.is_active && p.id !== currentPlanId && p.sort_order > (currentPlanSortOrder ?? 0),
    )
  ...
  const currentPlanData = (Array.isArray(plansQ.data) ? plansQ.data : []).find(
    (p: any) => p.id === currentPlanId,
  );
  ...
  const changeReqMut = useMutation({
    mutationFn: (body: { company_id: string; requested_plan_id: string; reason?: string }) => {
      ...
      return api
        .post(`/subscription/change-requests?${params.toString()}`, {
  ...
  const userDiff =
    selectedPlan && currentPlanData
      ? selectedPlan.included_users - currentPlanData.included_users
      : null;
  const millDiff =
    selectedPlan && currentPlanData
      ? selectedPlan.included_mills - currentPlanData.included_mills
      : null;
          ...
          {selectedPlan && (userDiff !== null || millDiff !== null) && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-[12px] text-blue-800">
              ...
              <ul className="space-y-0.5 text-[11px]">
                {userDiff !== null && userDiff > 0 && (
                  <li>
                    +{userDiff} users ({selectedPlan.included_users} total)
                  </li>
                )}
                {millDiff !== null && millDiff > 0 && (
                  <li>
                    +{millDiff} mills ({selectedPlan.included_mills} total)
                  </li>
                )}
    ...
    <div className="bg-white border border-[#e2e8f0] rounded-lg p-5">
      ...
      {isOver && (
        <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
          You have exceeded your {label.toLowerCase()} limit by {overage}.{" "}
          {overageCostLabel && `Purchase more at ${overageCostLabel}.`}
        </div>
      )}
    </div>
  );
...
function MillOwnerBillingView() {
  ...
  const [overageResource, setOverageResource] = useState<
    "extra_users" | "extra_mills" | "extra_employees"
  >("extra_users");
      ...
      <div className="flex flex-col min-h-full bg-[#f8fafc]">
        ...
        <div className="p-6 space-y-4 animate-pulse max-w-5xl mx-auto w-full">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-28 bg-white border border-[#e2e8f0] rounded-xl" />
            ))}
          </div>
    ...
    <div className="flex flex-col min-h-full bg-[#f8fafc]">
      ...
      <div className="p-4 md:p-6 space-y-5 max-w-5xl mx-auto w-full">
          ...
          <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-blue-600 mt-0.5">
                Requested{" "}
                {d.pending_upgrade.requested_at
                  ? new Date(d.pending_upgrade.requested_at).toLocaleDateString("en-IN", {
                      day: "numeric",
                      month: "short",
                      year: "numeric",
                    })
        ...
        {(atLimit || nearLimit) && !d.pending_upgrade && (
          ...
          >
            <AlertTriangle
              className={cn("w-5 h-5 shrink-0 mt-0.5", atLimit ? "text-red-500" : "text-amber-500")}
            />
            <div className="flex-1 min-w-0">
              <p
                className={cn("font-semibold text-sm", atLimit ? "text-red-800" : "text-amber-800")}
              >
                {atLimit
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap gap-4 text-xs text-[#64748b] mt-2">
                {d.next_billing_at && (
                  <span>
                    Next billing:{" "}
                    <strong className="text-[#374151]">{fmtDate(d.next_billing_at)}</strong>
                  </span>
                )}
                {d.last_payment_at && (
                  <span>
                    Last payment:{" "}
                    <strong className="text-[#374151]">{fmtDate(d.last_payment_at)}</strong>
                  </span>
                )}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <UsageBarCard
            ...
            onPurchase={() => {
              setOverageResource("extra_users");
              setOverageOpen(true);
            }}
          />
          <UsageBarCard
            ...
            onPurchase={() => {
              setOverageResource("extra_mills");
              setOverageOpen(true);
            }}
          />
          <UsageBarCard
            ...
            onPurchase={() => {
              setOverageResource("extra_employees");
              setOverageOpen(true);
            }}
          />
        </div>
                  ...
                  >
                    <div
                      className={cn(
                        ...
                        mod.enabled ? "bg-green-100" : "bg-gray-100",
                      )}
                    >
                      <Icon
                        className={cn(
                          "w-3.5 h-3.5",
                          mod.enabled ? "text-green-600" : "text-[#94a3b8]",
                        )}
                      />
                    </div>
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {(d.mills ?? []).length === 0 ? (
                <p className="text-xs text-[#94a3b8] text-center py-6">
                  No mills yet. Add your first mill.
                </p>
              ) : (
                (d.mills ?? []).map((mill: any) => (
                  <div
                    key={mill.id}
                    className="flex items-center gap-2 px-3 py-2.5 rounded-lg border border-[#e2e8f0] hover:bg-[#f8fafc]"
                  >
                    <span className="font-mono text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-semibold shrink-0">
                      {mill.code}
                    </span>
                ...
                <tbody>
                  {(d.invoices ?? []).map((inv: any) => (
                    <tr key={inv.id} className="border-b border-[#f1f5f9] hover:bg-[#f8fafc]">
                      ...
                      <td className="px-4 py-3 font-mono text-[#0f172a] whitespace-nowrap">
                        {fmtLakh(inv.amount)}
                      </td>
                      <td className="px-4 py-3">
        ...
        <div className="bg-white border border-[#e2e8f0] rounded-xl p-5">
          ...
          <p className="text-xs text-[#64748b] mb-3">
            Changes the currency symbol shown throughout the ERP. Values are not converted.
          </p>
            ...
            <div className="px-6 py-4 space-y-3">
              {[
                ...
                <div key={key}>
                  ...
                  />
                  {millFormErrors[key] && (
                    <p className="text-xs text-red-600 mt-1">{millFormErrors[key]}</p>
                  )}
                </div>
              ))}
            </div>
