...
export const FIELD_ALIASES: Record<string, string[]> = {
  ...
  section: ["section", "section name", "sub dept", "sub-dept", "unit", "floor", "area"],
  food_allowance: [
    "food allow",
    "food",
    "meal allowance",
  ...
  festival_holiday_allowance: [
    "festival holiday allowance",
    "fha",
    "festival allowance",
    "holiday allowance",
  ],
  ...
  for (const header of excelHeaders) {
    ...
    for (const [fieldKey, aliases] of Object.entries(FIELD_ALIASES)) {
      for (const alias of aliases) {
        ...
        if ((normalizedHeader.includes(na) || na.includes(normalizedHeader)) && na.length >= 3) {
    ...
    for (const col of columnConfigs) {
      ...
      const threshold = Math.max(
        3,
        Math.floor((normalizedHeader.length * (100 - minConfidence)) / 100),
      );
...
export function filterBlankRows(
  rows: any[],
  keyFields: string[] = ["name", "employee_id", "sl_no"],
): any[] {
...
export async function generateImportTemplate(
  ...
  const examples = visible.map((c) =>
    c.type === "date" ? "2024-01-25" : c.type === "number" ? "0" : (c.defaultValue ?? ""),
  );
