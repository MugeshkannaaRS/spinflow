...
function QualityPage() {
  ...
  const testsQ = useQuery({ queryKey: ["quality-tests"], queryFn: qualityApi.getTests });
  const lotsQ = useQuery({ queryKey: ["lot-approvals"], queryFn: qualityApi.getApprovals });
  const rejQ = useQuery({ queryKey: ["rejections"], queryFn: qualityApi.getRejections });

  const tests = testsQ.data ?? [];
  const lots = lotsQ.data ?? [];
  const rejections = rejQ.data ?? [];

  const [filteredTests, setFilteredTests] = useState(tests);
  const [filteredLots, setFilteredLots] = useState(lots);
  const [filteredRejections, setFilteredRejections] = useState(rejections);

  useEffect(() => { setFilteredTests(tests); }, [tests]);
  useEffect(() => { setFilteredLots(lots); }, [lots]);
  useEffect(() => { setFilteredRejections(rejections); }, [rejections]);
  ...
  const totalRejectedKg = rejections.reduce((s, r) => s + r.quantityKg, 0);
              ...
              <CardContent>
                <ExcelColumnFilter
                  ...
                  columns={[
                    ...
                    { key: "sampleRef" as const, label: "Sample", placeholder: "Filter sample..." },
                    { key: "status" as const, label: "Status", placeholder: "Filter status..." },
                    { key: "testedBy" as const, label: "Tested By", placeholder: "Filter tester..." },
                  ]}
                />
                <Table>
                  <TableHeader>
              ...
              <CardContent>
                <ExcelColumnFilter
                  ...
                  columns={[
                    { key: "date" as const, label: "Date", placeholder: "Filter date..." },
                    { key: "lotId" as const, label: "Lot", placeholder: "Filter lot..." },
                    { key: "category" as const, label: "Category", placeholder: "Filter category..." },
                    { key: "reason" as const, label: "Reason", placeholder: "Filter reason..." },
                    { key: "disposition" as const, label: "Disposition", placeholder: "Filter disposition..." },
                    { key: "notedBy" as const, label: "Noted By", placeholder: "Filter noted by..." },
                  ]}
                />
                <Table>
                  <TableHeader>
