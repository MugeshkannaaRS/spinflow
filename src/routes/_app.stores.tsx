...
function StoresPage() {
  ...
  const itemsQ = useQuery({ queryKey: ["spare-items"], queryFn: storesApi.getSpares });
  const issuesQ = useQuery({ queryKey: ["issue-notes"], queryFn: storesApi.getIssues });

  const items = itemsQ.data ?? [];
  const issues = issuesQ.data ?? [];

  const [itemFiltered, setItemFiltered] = useState(items);
  const [issueFiltered, setIssueFiltered] = useState(issues);
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card>
            <CardContent className="p-5">
              <div className="text-xs uppercase text-muted-foreground font-medium">Spare Items</div>
          ...
          <Card>
            <CardContent className="p-5">
              <div className="text-xs uppercase text-muted-foreground font-medium">Total Stock</div>
              <div className="text-2xl font-semibold mt-2 flex items-center gap-2">
          ...
          <Card>
            <CardContent className="p-5">
              <div className="text-xs uppercase text-muted-foreground font-medium">Low Stock Items</div>
              <div className="text-2xl font-semibold mt-2 flex items-center gap-2">
          ...
          <Card>
            <CardContent className="p-5">
              <div className="text-xs uppercase text-muted-foreground font-medium">Needs Reorder</div>
        ...
        <Tabs defaultValue="inventory">
          ...
          <TabsContent value="inventory">
            <Card>
              ...
              <CardContent>
                <ExcelColumnFilter data={items} onFilter={setItemFiltered} columns={itemColumns} />
                <Table>
                  <TableHeader>
                    <TableRow>
          ...
          <TabsContent value="issues">
            <Card>
              ...
              <CardContent>
                <ExcelColumnFilter data={issues} onFilter={setIssueFiltered} columns={issueColumns} />
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {issueFiltered.map((n) => (
                      <TableRow key={n.id}>
                        ...
                        <TableCell><span className="font-mono text-xs">{n.itemCode}</span> — {n.itemName}</TableCell>
