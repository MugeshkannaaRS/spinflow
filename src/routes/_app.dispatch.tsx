...
function DispatchPage() {
  ...
  const ordersQ = useQuery({ queryKey: ["sales-orders"], queryFn: dispatchApi.getOrders });
  const dispatchQ = useQuery({ queryKey: ["dispatches"], queryFn: dispatchApi.getOrders });

  const orders = ordersQ.data ?? [];
  const dispatches = dispatchQ.data ?? [];

  const [filteredOrders, setFilteredOrders] = useState(orders);
  const [filteredDispatches, setFilteredDispatches] = useState(dispatches);

  useEffect(() => { setFilteredOrders(orders); }, [orders]);
  useEffect(() => { setFilteredDispatches(dispatches); }, [dispatches]);

  const pendingOrders = orders.filter((o) => o.status === "pending").length;
  const activeDispatches = dispatches.filter(
    (d) => d.status === "loaded" || d.status === "pending",
  ).length;
  const totalDispatched = dispatches.reduce((s, d) => s + d.quantityKg, 0);
            ...
            <Card>
              ...
              <CardContent>
                <ExcelColumnFilter
                  ...
                  columns={[
                    { key: "orderNo" as const, label: "Order No", placeholder: "Filter order..." },
                    { key: "customer" as const, label: "Customer", placeholder: "Filter customer..." },
                    { key: "date" as const, label: "Date", placeholder: "Filter date..." },
                    { key: "deliveryDate" as const, label: "Delivery", placeholder: "Filter delivery..." },
            ...
            <Card>
              ...
              <CardContent>
                <ExcelColumnFilter
                  ...
                  columns={[
                    { key: "dispatchNo" as const, label: "Dispatch No", placeholder: "Filter dispatch..." },
                    { key: "date" as const, label: "Date", placeholder: "Filter date..." },
                    { key: "orderNo" as const, label: "Order", placeholder: "Filter order..." },
                    { key: "customer" as const, label: "Customer", placeholder: "Filter customer..." },
                    { key: "lotNo" as const, label: "Lot", placeholder: "Filter lot..." },
                    { key: "vehicleNo" as const, label: "Vehicle", placeholder: "Filter vehicle..." },
                    { key: "ewayBillNo" as const, label: "E-Way Bill", placeholder: "Filter e-way bill..." },
                    { key: "status" as const, label: "Status", placeholder: "Filter status..." },
                  ]}
                />
                <Table>
                  ...
                  <TableBody>
                    {filteredDispatches.map((d) => (
                      <TableRow key={d.id}>
                        ...
                        <TableCell className="font-mono text-xs">{d.ewayBillNo || "—"}</TableCell>
                        <TableCell>
                          <Badge
                            variant={
