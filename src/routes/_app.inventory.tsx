...
function InventoryPage() {
  ...
  const canEdit = canAccess("inventory", true);
  ...
  const lotsQ = useQuery({
    queryKey: ["inventory-lots", millId],
  ...
  const transfersQ = useQuery({
    queryKey: ["stock-transfers", millId],
  ...
  const lotCols: ColDef[] = [
    ...
    {
      key: "type",
      label: lotColConfig.getLabel("type"),
      type: "status",
      render: (l: any) => <StatusBadge status="idle" label={l.type} size="sm" />,
    },
    { key: "department", label: lotColConfig.getLabel("department"), type: "status" },
    {
      key: "quantity",
      label: lotColConfig.getLabel("quantity"),
      render: (l: any) => <span className="text-right">{(l.quantity ?? 0).toLocaleString()}</span>,
    },
    { key: "location", label: lotColConfig.getLabel("location"), type: "status" },
    {
      key: "grade",
      label: lotColConfig.getLabel("grade"),
      type: "status",
      render: (l: any) => (
        <Badge
          variant={
    ...
    {
      key: "age",
      label: lotColConfig.getLabel("age"),
      render: (l: any) => (
        <span className={l.age > 14 ? "text-destructive font-medium" : ""}>{l.age}</span>
      ),
    },
    {
      key: "status",
  ...
  const transferCols: ColDef[] = [
    ...
    {
      key: "toLocation",
      label: transferColConfig.getLabel("to_location"),
      type: "status",
      render: (t: any) => (
        <span>
    ...
    {
      key: "quantity",
      label: transferColConfig.getLabel("quantity"),
      render: (t: any) => `${t.quantity} ${t.unit}`,
    },
    { key: "transferredBy", label: transferColConfig.getLabel("transferred_by") },
    {
      key: "status",
      label: transferColConfig.getLabel("status"),
      type: "status",
...
function NewTransferDialog() {
  ...
  const f = (key: keyof typeof form) => ({
    ...
    onChange: (e: React.ChangeEvent<HTMLInputElement>) => {
      setForm({ ...form, [key]: key === "quantity" ? +e.target.value : e.target.value });
      ...
      <DialogContent className="w-full max-w-lg mx-4 overflow-y-auto max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            {(
              ["date", "lotNo", "fromLocation", "toLocation", "unit", "transferredBy"] as const
            ).map((key) => (
              <div key={key} className="space-y-1.5">
                <Label>
                  {getTransferLabel(key.replace(/([A-Z])/g, "_$1").toLowerCase())}{" "}
            ...
            <div className="space-y-1.5">
              <Label>
                {getTransferLabel("quantity")} <span className="text-destructive">*</span>
              </Label>
