...
function PlanBadge({ plan }: { plan: string }) {
  ...
  return (
    <span
      className={`text-[10px] font-semibold px-2 py-0.5 rounded-full capitalize ${colors[plan?.toLowerCase()] ?? "bg-gray-100 text-gray-700"}`}
    >
      {plan ?? "N/A"}
    </span>
  );
...
function AnalyticsCenterPage() {
  ...
  const growth: any[] = Array.isArray(companyGrowthQ.data?.data ?? companyGrowthQ.data)
    ? (companyGrowthQ.data?.data ?? companyGrowthQ.data)
    : [];
  const adoption: any[] = Array.isArray(moduleAdoptionQ.data?.data ?? moduleAdoptionQ.data)
    ? (moduleAdoptionQ.data?.data ?? moduleAdoptionQ.data)
    : [];
  const retention: any[] = Array.isArray(retentionQ.data?.data ?? retentionQ.data)
    ? (retentionQ.data?.data ?? retentionQ.data)
    : [];
  ...
  const customerHealth = useMemo(() => {
    const healthy = subsList.filter(
      (s: any) => (s.user_count ?? 0) < (s.user_limit ?? 1) * 0.8 && s.status === "active",
    ).length;
    const atRisk = subsList.filter(
      (s: any) => (s.user_count ?? 0) >= (s.user_limit ?? 1) * 0.85 || s.status === "overdue",
    ).length;
    const churned = subsList.filter(
      (s: any) => s.status === "suspended" || s.status === "cancelled",
    ).length;
    ...
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Revenue, growth, retention, and adoption analytics
          </p>
        </div>
        ...
        {anyLoading && !an ? (
          ...
          <>
            <KpiCard
              ...
              value={
                an
                  ? `₹${an.mrr?.toLocaleString("en-IN", { maximumFractionDigits: 0 }) ?? "—"}`
                  : "—"
              }
            ...
            <KpiCard
              ...
              value={
                an
                  ? `₹${an.arr?.toLocaleString("en-IN", { maximumFractionDigits: 0 }) ?? "—"}`
                  : "—"
              }
              subLabel="Annual recurring"
              icon={DollarSign}
            ...
            <KpiCard
              ...
              value={
                an
                  ? `₹${(an.arpu ?? 0).toLocaleString("en-IN", { maximumFractionDigits: 0 })}`
                  : "—"
              }
              subLabel="Per user"
              icon={Users}
            ...
            <KpiCard
              label="LTV"
              value={
                an ? `₹${(an.ltv ?? 0).toLocaleString("en-IN", { maximumFractionDigits: 0 })}` : "—"
              }
              subLabel="Customer lifetime"
              icon={HeartHandshake}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm flex items-center gap-2">
              <BarChart3 className="size-4" /> Company Growth ({timeRange})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {companyGrowthQ.isLoading ? (
                ...
                {growth.slice(0, 12).map((g: any, i: number) => (
                  <div key={i} className="flex items-center gap-3 text-xs">
                    <span className="w-24 text-muted-foreground">
                      {g.period ?? g.month ?? g.label}
                    </span>
                    <div className="flex-1 h-4 rounded bg-gray-100">
                      <div
                        className="h-4 rounded bg-blue-500"
                        style={{
                          width: `${Math.min(((g.count ?? g.value ?? 0) / Math.max(...growth.map((x: any) => x.count ?? x.value ?? 0), 1)) * 100, 100)}%`,
                        }}
                      />
                    </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm flex items-center gap-2">
              <Lightbulb className="size-4" /> Module Adoption
            </CardTitle>
          </CardHeader>
          <CardContent>
            {moduleAdoptionQ.isLoading ? (
              ...
              <p className="text-sm text-muted-foreground py-8 text-center">
                No module adoption data.
              </p>
            ) : (
              <div className="space-y-2">
                {adoption.slice(0, 15).map((m: any, i: number) => (
                  <div key={i} className="flex items-center gap-3 text-xs">
                    <span className="w-28 text-muted-foreground capitalize truncate">
                      {m.module ?? m.name ?? m.code}
                    </span>
                    <div className="flex-1 h-4 rounded bg-gray-100">
                      <div
                        className={`h-4 rounded ${(m.adoption_rate ?? m.percentage ?? 0) > 75 ? "bg-emerald-500" : (m.adoption_rate ?? 0) > 50 ? "bg-blue-500" : "bg-amber-400"}`}
                        style={{ width: `${Math.min(m.adoption_rate ?? m.percentage ?? 0, 100)}%` }}
                      />
                    </div>
                    <span className="w-10 text-right font-medium">
                      {m.adoption_rate ?? m.percentage ?? 0}%
                    </span>
                  </div>
                ))}
              </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm flex items-center gap-2">
              <Users className="size-4" /> Retention Cohort
            </CardTitle>
          </CardHeader>
          <CardContent>
            {retentionQ.isLoading ? (
                    ...
                    {retention.slice(0, 10).map((r: any, i: number) => (
                      <tr key={i} className="border-b border-gray-50">
                        <td className="px-2 py-1.5 font-medium">
                          {r.cohort ?? r.period ?? r.month}
                        </td>
                        <td className="px-2 py-1.5 text-center">{r.size ?? r.count ?? 0}</td>
                        <td className="px-2 py-1.5 text-center">
                          <span
                            className={`font-medium ${(r.month_1 ?? r.retention_1m ?? 0) >= 80 ? "text-green-600" : (r.month_1 ?? 0) >= 60 ? "text-amber-600" : "text-red-600"}`}
                          >
                        ...
                        <td className="px-2 py-1.5 text-center">
                          <span
                            className={`font-medium ${(r.month_3 ?? r.retention_3m ?? 0) >= 70 ? "text-green-600" : "text-amber-600"}`}
                          >
                        ...
                        <td className="px-2 py-1.5 text-center">
                          <span
                            className={`font-medium ${(r.month_6 ?? r.retention_6m ?? 0) >= 60 ? "text-green-600" : "text-amber-600"}`}
                          >
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm flex items-center gap-2">
              <DollarSign className="size-4" /> MRR Breakdown
            </CardTitle>
          </CardHeader>
            ...
            {mrrBreakdownQ.isLoading ? (
              ...
              <>
                {[
                  {
                    label: "Subscription",
                    value: mrrBreakdown.subscription ?? 0,
                    color: "bg-blue-500",
                  },
                  ...
                  const total =
                    (mrrBreakdown.subscription ?? 0) +
                    (mrrBreakdown.overage ?? 0) +
                    (mrrBreakdown.addon ?? 0);
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm flex items-center gap-2">
              <Crown className="size-4" /> Plan Distribution
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm flex items-center gap-2">
              <HeartHandshake className="size-4" /> Customer Health
            </CardTitle>
          </CardHeader>
          <CardContent>
            ...
            {customerHealth.total > 0 && (
              <div className="h-3 rounded-full bg-gray-100 flex overflow-hidden">
                <div
                  ...
                  style={{ width: `${(customerHealth.healthy / customerHealth.total) * 100}%` }}
                />
                <div
                  className="bg-amber-400 h-3"
                  style={{ width: `${(customerHealth.atRisk / customerHealth.total) * 100}%` }}
                />
                <div
                  className="bg-red-500 h-3"
                  style={{ width: `${(customerHealth.churned / customerHealth.total) * 100}%` }}
                />
              </div>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm flex items-center gap-2">
              <Building2 className="size-4" /> Company Ranking by MRR
            </CardTitle>
          </CardHeader>
          <CardContent>
                ...
                {companyRanking.map((s: any, i: number) => (
                  <div key={s.company_id} className="flex items-center gap-3 text-xs">
                    ...
                    <span className="w-24 text-right font-medium">
                      ₹
                      {(s.mrr ?? s.revenue ?? 0).toLocaleString("en-IN", {
