    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          License overview, user provisioning, and subscription enforcement.
        </p>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {companies.filter((c: any) => c.is_active !== false).length}
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {companyStats.reduce((a: number, s: any) => a + (s.user_count ?? 0), 0)}
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {companyStats.reduce((a: number, s: any) => a + (s.mill_count ?? 0), 0)}
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              columns={
                [
                    ...
                    render: (c: any) => {
                      ...
                      return (
                        <div className="flex items-center gap-2">
                          ...
                          {pct >= 100 ? (
                            <span className="text-xs font-semibold text-red-600 bg-red-50 dark:bg-red-900/30 dark:text-red-400 px-1.5 py-0.5 rounded">
                              FULL
                            </span>
                          ) : pct >= 90 ? (
                            <span className="text-xs font-semibold text-amber-600 bg-amber-50 dark:bg-amber-900/30 dark:text-amber-400 px-1.5 py-0.5 rounded">
                              {Math.round(pct)}%
                            </span>
                          ) : null}
                        </div>
                  ...
                  },
                  {
                    key: "_mills",
                    label: "Mills",
                    render: (c: any) => statsMap.get(c.id)?.mill_count ?? 0,
                  },
                  {
                    key: "plan",
                    label: "Plan",
                    render: (c: any) => {
                      ...
                      return (
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${PLAN_COLORS[plan] ?? PLAN_COLORS.starter}`}
                        >
                          {plan}
                        </span>
                      );
