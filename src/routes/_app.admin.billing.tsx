...
function BillingPage() {
  ...
  if (summaryQ.isError) {
    return (
      <div className="p-6 space-y-6">
        <div><h1 className="text-2xl font-bold">Billing</h1><p className="text-sm text-muted-foreground mt-1">Commercial control center — subscriptions, invoicing, and revenue analytics.</p></div>
        <div className="rounded-lg border border-red-200 bg-red-50 dark:bg-red-900/10 dark:border-red-800 p-8 text-center">
          ...
          <p className="text-sm font-medium text-red-700 dark:text-red-400">Failed to load billing summary.</p>
          <p className="text-xs text-red-500 mt-1 mb-3">{(summaryQ.error as any)?.response?.data?.detail ?? (summaryQ.error as any)?.message ?? "Request failed"}</p>
          <Button variant="outline" size="sm" onClick={() => summaryQ.refetch()}>Retry</Button>
        </div>
        <SectionGrid />
      </div>
    );
  ...
  const kpis = [
    ...(summaryQ.isLoading
      ? Array.from({ length: 6 }).map((_, i) => ({ label: "", value: "—", icon: TrendingUp, color: "text-muted-foreground", sub: "", key: i }))
      : [
        { label: "MRR", value: `₹${(d!.mrr ?? 0).toLocaleString("en-IN", { maximumFractionDigits: 0 })}`, icon: TrendingUp, color: "text-blue-600", sub: "Monthly Recurring Revenue" },
        { label: "ARR", value: `₹${(d!.arr ?? 0).toLocaleString("en-IN", { maximumFractionDigits: 0 })}`, icon: DollarSign, color: "text-green-600", sub: "Annual Recurring Revenue" },
        { label: "Active", value: String(d!.active_subscriptions ?? 0), icon: CheckCircle2, color: "text-emerald-600", sub: "Active subscriptions" },
        { label: "Overdue", value: String(d!.overdue_count ?? 0), icon: AlertTriangle, color: "text-red-600", sub: "Overdue accounts" },
        { label: "Trial", value: String(d!.trial_count ?? 0), icon: UsersIcon, color: "text-amber-600", sub: "Trial accounts" },
        { label: "Collection", value: `${d!.collection_rate ?? 0}%`, icon: CheckCircle2, color: "text-purple-600", sub: "Collection rate" },
      ]
    ),
  ];
