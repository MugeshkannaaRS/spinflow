...
function ApprovalCenterPage() {
  ...
  const requestsQ = useQuery({
    ...
    queryFn: () =>
      adminApi.getApprovalRequests({
        status: tab === "escalated" ? "escalated" : tab,
  ...
  const requests: any[] = Array.isArray(
    requestsQ.data?.items ?? requestsQ.data?.data ?? requestsQ.data,
  )
    ? (requestsQ.data?.items ?? requestsQ.data?.data ?? requestsQ.data)
    : [];
  ...
  const tabCounts = {
    ...
    escalated:
      requestsQ.data?.total ?? requests.filter((r: any) => r.status === "escalated").length,
  };
    ...
    <div className="p-4 md:p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Review and manage approval requests across companies
        </p>
      </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium capitalize">
                  {selectedRequest.approval_type ?? selectedRequest.type ?? "—"}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">
                  {selectedRequest.requested_by_name ?? selectedRequest.requested_by ?? "—"}
                </p>
              </div>
              <div>
            ...
            {selectedRequest.reason && (
              <div>
                ...
                <p className="text-sm mt-0.5">{selectedRequest.reason}</p>
              </div>
            )}
              ...
              {tab === "pending" && (
                <>
                  <Button
                    size="sm"
                    onClick={() =>
                      actionMutation.mutate({ id: selectedRequest.id, action: "approve" })
                    }
                  ...
                  <Button
                    ...
                    variant="destructive"
                    onClick={() =>
                      actionMutation.mutate({ id: selectedRequest.id, action: "reject" })
                    }
                    disabled={actionMutation.isPending}
                  >
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      actionMutation.mutate({ id: selectedRequest.id, action: "escalate" })
                    }
                    disabled={actionMutation.isPending}
                  >
      ...
      {requestsQ.isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            All clear — no {tab} requests to review.
          </p>
        </div>
      ) : (
        <div className="space-y-2">
          {requests.map((req: any) => (
            <Card
              key={req.id}
              className={`cursor-pointer transition-colors hover:shadow-sm ${selectedRequest?.id === req.id ? "ring-2 ring-blue-200" : ""}`}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="font-semibold text-sm capitalize">
                        {req.approval_type ?? req.type ?? "Approval Request"}
                      </p>
                      <StatusBadge status={req.status} />
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">
                      {req.company_name && <>{req.company_name} · </>}
                      {req.requested_by_name ?? req.requested_by ?? "Unknown"} ·{" "}
