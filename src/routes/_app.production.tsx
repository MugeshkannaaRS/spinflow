...
function ProductionPage() {
  ...
  const machinesQ = useQuery({ queryKey: ["machines"], queryFn: productionApi.getMachines });
  const shiftsQ = useQuery({ queryKey: ["shifts"], queryFn: productionApi.getEntries });
  const downQ = useQuery({ queryKey: ["downtime"], queryFn: productionApi.getDowntime });
  ...
  const [machineFiltered, setMachineFiltered] = useState(machines);
  const [shiftFiltered, setShiftFiltered] = useState(shifts);
  const [downFiltered, setDownFiltered] = useState(downtime);
  ...
  const totalProduced = machines.reduce((s: number, m: any) => s + (m.producedKg ?? m.produced_kg ?? 0), 0);
  const totalTarget = machines.reduce((s: number, m: any) => s + (m.targetKg ?? m.target_kg ?? 0), 0);
  const running = machines.filter((m: any) => (m.status ?? m.current_status) === "running").length;
  const breakdown = machines.filter((m: any) => (m.status ?? m.current_status) === "breakdown").length;
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card>
            <CardContent className="p-5">
              ...
              <div className="text-2xl font-semibold mt-2">{totalProduced.toLocaleString()} kg</div>
              <Progress value={(totalProduced / totalTarget) * 100} className="h-1.5 mt-3" />
          ...
          <Card>
            <CardContent className="p-5">
              ...
              <div className="text-2xl font-semibold mt-2">
                {machines.length
                  ? Math.round(machines.reduce((s, m) => s + m.efficiency, 0) / machines.length)
                  : 0}
                %
              </div>
            </CardContent>
        ...
        <Tabs defaultValue="machines">
          ...
          <TabsContent value="machines">
            <Card>
              ...
              <CardContent>
                <ExcelColumnFilter data={machines} onFilter={setMachineFiltered} columns={machineColumns} />
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      {mvk.has("targetKg") && <TableHead className="text-right">Target (kg)</TableHead>}
                      {mvk.has("producedKg") && <TableHead className="text-right">Produced (kg)</TableHead>}
                      {mvk.has("efficiency") && <TableHead className="text-right">Efficiency</TableHead>}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {machineFiltered.map((m) => (
                      <TableRow key={m.id}>
                        {mvk.has("code") && <TableCell className="font-mono text-xs">{m.code}</TableCell>}
                        {mvk.has("department") && <TableCell>{m.department}</TableCell>}
                        {mvk.has("status") && <TableCell>
                          <Badge
                            variant={
                              m.status === "running"
                                ? "default"
                        ...
                        {mvk.has("targetKg") && <TableCell className="text-right">{m.targetKg}</TableCell>}
                        {mvk.has("producedKg") && <TableCell className="text-right">{m.producedKg}</TableCell>}
                        {mvk.has("efficiency") && <TableCell className="text-right">
                          <span
                            className={
                              m.efficiency >= 85
          ...
          <TabsContent value="shifts">
            <Card>
              ...
              <CardContent>
                <ExcelColumnFilter data={shifts} onFilter={setShiftFiltered} columns={shiftColumns} />
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      {svk.has("producedKg") && <TableHead className="text-right">Produced</TableHead>}
                      {svk.has("wasteKg") && <TableHead className="text-right">Waste</TableHead>}
                  ...
                  <TableBody>
                    {shiftFiltered.map((s) => (
                      <TableRow key={s.id}>
                        ...
                        {svk.has("shift") && <TableCell>
                          <Badge variant="outline">{s.shift}</Badge>
                        </TableCell>}
                        {svk.has("machineCode") && <TableCell className="font-mono text-xs">{s.machineCode}</TableCell>}
                        ...
                        {svk.has("producedKg") && <TableCell className="text-right">{s.producedKg} kg</TableCell>}
                        {svk.has("wasteKg") && <TableCell className="text-right text-muted-foreground">
                          {s.wasteKg} kg
                        </TableCell>}
                        {svk.has("status") && <TableCell>
                          <Badge
                            variant={
                              s.status === "approved"
                                ? "default"
                          ...
                          >
                            {s.status === "approved" && <CheckCircle2 className="size-3 mr-1" />}
                            {s.status}
                          </Badge>
                        </TableCell>}
                      </TableRow>
          ...
          <TabsContent value="downtime">
            <Card>
              ...
              <CardContent>
                <ExcelColumnFilter data={downtime} onFilter={setDownFiltered} columns={downColumns} />
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      {dvk.has("durationMin") && <TableHead className="text-right">Duration</TableHead>}
                  ...
                  <TableBody>
                    {downFiltered.map((d) => (
                      <TableRow key={d.id}>
                        {dvk.has("machineCode") && <TableCell className="font-mono text-xs">{d.machineCode}</TableCell>}
                        {dvk.has("reason") && <TableCell>{d.reason}</TableCell>}
                        {dvk.has("startedAt") && <TableCell>{d.startedAt}</TableCell>}
                        {dvk.has("durationMin") && <TableCell className="text-right">{d.durationMin} min</TableCell>}
                        {dvk.has("resolved") && <TableCell>
...
const DEPARTMENTS = [
  "Blowroom", "Carding", "Drawing", "Simplex", "Ring Frame", "Winding", "Quality",
];
