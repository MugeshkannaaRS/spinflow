...
const PAGE_SIZE = 25;
type SortKey = "name" | "role" | "company" | "mill" | "status" | "last_login" | "created_at";
...
function AdminUsersPage() {
  ...
  const usersQ = useQuery({
    ...
    queryFn: () =>
      api.get("/admin/users", { params: { page_size: 2000 } }).then((r) => r.data?.items ?? []),
    staleTime: 30_000,
  });
  ...
  const allRoles = useMemo(
    () => [...new Set(allUsers.map((u: any) => u.role))].sort(),
    [allUsers],
  );
    ...
    let out = allUsers.filter((u) => {
      if (
        ...
        !(
          ...
          u.mill_name?.toLowerCase().includes(q) ||
  ...
  const StatCard = ({
    ...
    <button
      onClick={onClick}
      className={cn(
        "text-left bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-4 transition-all",
        onClick ? "hover:border-blue-300 hover:shadow-sm cursor-pointer" : "cursor-default",
      )}
    >
      ...
      <p className={cn("text-3xl font-bold tabular-nums", usersQ.isLoading ? "opacity-40" : "")}>
        {usersQ.isLoading ? "—" : value.toLocaleString()}
      </p>
    </button>
  );
    ...
    <div className="p-6 space-y-5 max-w-[1400px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          icon={UserMinus} color="bg-amber-50 text-amber-600"
          onClick={() => { setStatusFilter("inactive"); setPage(1); }}
        />
        <StatCard
          label="Mill Owners" value={ownerCount}
          icon={ShieldCheck} color="bg-purple-50 text-purple-600"
          onClick={() => { setRoleFilter("MILL_OWNER"); setPage(1); }}
        />
      </div>
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              ...
              placeholder="Search name, email, company, mill…"
          ...
          {hasFilter && (
            <button
              onClick={() => {
                setSearch(""); setCompanyFilter("all");
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-sm">
              ...
              <tr className="bg-gray-50 dark:bg-slate-800/60 border-b border-gray-200 dark:border-slate-700">
                {(
                  ...
                  <th
                    ...
                    className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide cursor-pointer select-none hover:text-foreground whitespace-nowrap transition-colors"
                  >
                    {col.label}
                    ...
                    {[...Array(8)].map((_, j) => (
                      <td key={j} className="px-4 py-3.5">
                        <div
                          ...
                          style={{
                            width: j === 0 ? "160px" : j === 1 ? "90px" : j === 2 ? "130px" : "70px",
                          }}
                        />
                      </td>
                    ))}
        ...
        {filtered.length > PAGE_SIZE && (
          <div className="px-4 py-3 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between text-xs text-muted-foreground bg-gray-50 dark:bg-slate-800/40">
            <span>
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                disabled={page === 1}
                className="px-2.5 py-1 border border-gray-200 dark:border-slate-600 rounded-lg disabled:opacity-40 hover:bg-white dark:hover:bg-slate-700 transition-colors"
              >
                ‹
              </button>
            ...
            <div className="space-y-1.5">
              ...
              {form.password && (
                <p className={cn("text-xs", pwOk(form.password) ? "text-emerald-600" : "text-amber-600")}>
                  {pwOk(form.password) ? "✓ Strong password" : "Need uppercase, lowercase, and a digit"}
                </p>
              )}
            </div>
            <div className="space-y-1.5">
