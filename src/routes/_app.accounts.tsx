...
import { Receipt, IndianRupee, AlertTriangle, CheckCircle2, TrendingUp, TrendingDown } from "lucide-react";
...
function AccountsPage() {
  ...
  const invQ = useQuery({ queryKey: ["invoices"], queryFn: accountsApi.getInvoices });
  const recvQ = useQuery({ queryKey: ["receivables"], queryFn: accountsApi.getReceivables });
  ...
  useEffect(() => { setFilteredReceivables(receivables); }, [receivables]);

  const totalSales = invoices.filter((i) => i.type === "sales").reduce((s, i) => s + i.total, 0);
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card>
            <CardContent className="p-5">
              <div className="text-xs uppercase text-muted-foreground font-medium">Total Sales</div>
              <div className="text-2xl font-semibold mt-2 flex items-center gap-2">
                <IndianRupee className="size-5 text-primary" />₹{(totalSales / 100000).toFixed(2)} L
              </div>
            </CardContent>
          </Card>
          <Card>
          ...
          <Card>
            <CardContent className="p-5">
              <div className="text-xs uppercase text-muted-foreground font-medium">Outstanding</div>
              <div className="text-2xl font-semibold mt-2 flex items-center gap-2">
            ...
            <Card>
              ...
              <CardContent>
                <ExcelColumnFilter
                  ...
                  columns={[
                    { key: "invoiceNo" as const, label: "Invoice No", placeholder: "Filter invoice..." },
                    { key: "date" as const, label: "Date", placeholder: "Filter date..." },
                    { key: "customer" as const, label: "Customer / Supplier", placeholder: "Filter customer..." },
                  ...
                  <TableBody>
                    {filteredInvoices.map((inv) => (
                      <TableRow key={inv.id}>
                        ...
                        <TableCell className="text-right">₹{inv.gst.toLocaleString()}</TableCell>
            ...
            <Card>
              ...
              <CardContent>
                <ExcelColumnFilter
                  ...
                  columns={[
                    { key: "customer" as const, label: "Customer", placeholder: "Filter customer..." },
                    { key: "invoiceNo" as const, label: "Invoice", placeholder: "Filter invoice..." },
                    { key: "status" as const, label: "Status", placeholder: "Filter status..." },
                  ]}
                />
                <Table>
                  ...
                  <TableBody>
                    {filteredReceivables.map((r) => (
                      <TableRow key={r.id}>
                        ...
                        <TableCell className="text-right">₹{(r.amount ?? 0).toLocaleString()}</TableCell>
  ...
  return (
    <div className="space-y-6">
      ...
      {pl && (
        <>
          <Card>
            <CardHeader><CardTitle className="text-base">Profit & Loss — {months[Number(month) - 1]} {year}</CardTitle></CardHeader>
            <CardContent>
              <div className="grid gap-4 md:grid-cols-4">
                ...
                <div className="p-3 rounded-lg bg-muted/50">
                  ...
                  <div className={`text-xl font-semibold flex items-center gap-1 ${pl.gross_profit >= 0 ? "text-green-600" : "text-red-600"}`}>
                    {pl.gross_profit >= 0 ? <TrendingUp className="size-4" /> : <TrendingDown className="size-4" />}
                    ₹{(pl.gross_profit / 100000).toFixed(2)} L
                  </div>
                  <div className="text-xs text-muted-foreground">Margin: {pl.gross_margin_pct}%</div>
                </div>
                <div className="p-3 rounded-lg bg-muted/50">
                  <div className="text-xs text-muted-foreground">Net Profit</div>
                  <div className={`text-xl font-semibold flex items-center gap-1 ${pl.net_profit >= 0 ? "text-green-600" : "text-red-600"}`}>
                    {pl.net_profit >= 0 ? <TrendingUp className="size-4" /> : <TrendingDown className="size-4" />}
              ...
              <div className="mt-4 h-24 flex items-end gap-4">
                {[
                  ...
                  { label: "Net Profit", val: pl.net_profit, color: pl.net_profit >= 0 ? "bg-green-500" : "bg-red-500" },
                ].map((b) => {
                  const maxVal = Math.max(pl.revenue, 1);
                  const h = Math.max((b.val / maxVal) * 100, 4);
                  return (
                    <div key={b.label} className="flex-1 flex flex-col items-center gap-1">
                      <div className="text-xs text-muted-foreground">₹{(b.val / 100000).toFixed(1)}L</div>
                      <div className={`w-full rounded ${b.color}`} style={{ height: `${h}px`, minHeight: 8 }} />
            ...
            <CardContent>
              {ageing ? (
                <div className="space-y-4">
                  <div className="text-3xl font-bold">₹{(ageing.total_outstanding / 100000).toFixed(2)} L</div>
                  <div className="text-sm text-muted-foreground">{ageing.invoice_count} invoices · Oldest: {ageing.oldest_invoice_days} days</div>
                  <div className="flex h-8 rounded-full overflow-hidden">
                    {[
                      ...
                      return (
                        <div key={b.label} className={`${b.color}`} style={{ width: `${pct}%` }} title={`${b.label}: ₹${b.val.toLocaleString()}`} />
                      );
                    })}
                  </div>
                  <div className="grid grid-cols-4 gap-2 text-xs">
                    {[
                      { label: "Current (0-30d)", val: ageing.buckets.current, color: "text-green-600" },
                      { label: "31-60 days", val: ageing.buckets.days_31_60, color: "text-yellow-600" },
                      { label: "61-90 days", val: ageing.buckets.days_61_90, color: "text-orange-600" },
          ...
          {/* GST Summary */}
          <Card>
            <CardHeader><CardTitle className="text-base">GST Summary — {months[Number(month) - 1]} {year}</CardTitle></CardHeader>
            <CardContent>
              {gst ? (
                <div className="grid gap-4 md:grid-cols-3">
                  <div className="p-3 rounded-lg bg-muted/50">
                    <div className="text-xs text-muted-foreground">Output GST</div>
                    <div className="text-lg font-semibold">₹{(gst.output_gst.total / 100000).toFixed(2)} L</div>
                    <div className="text-xs text-muted-foreground mt-1">
                      CGST: ₹{(gst.output_gst.cgst).toFixed(0)} · SGST: ₹{(gst.output_gst.sgst).toFixed(0)}
                      {gst.output_gst.igst > 0 && ` · IGST: ₹${gst.output_gst.igst.toFixed(0)}`}
                    </div>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/50">
                    <div className="text-xs text-muted-foreground">Input GST</div>
                    <div className="text-lg font-semibold">₹{(gst.input_gst.total / 100000).toFixed(2)} L</div>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/50">
                    <div className="text-xs text-muted-foreground">Net Payable</div>
                    <div className={`text-lg font-semibold ${gst.net_payable > 0 ? "text-destructive" : "text-green-600"}`}>
