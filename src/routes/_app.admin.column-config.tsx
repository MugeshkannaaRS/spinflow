...
function ColumnConfigPage() {
  ...
  const configQuery = useQuery({
    queryKey: ["column-config-admin", selectedTable],
    queryFn: async () => {
  ...
  useQuery({
    queryKey: ["dropdown-options", selectedTable],
    queryFn: async () => {
      // fetch dropdown options for first column that has type dropdown
      return null;
    },
    enabled: false,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const res = await api.put("/ui-config/columns", { columns }, {
        params: { table: selectedTable, mill_id: user?.millId ?? "default" },
      });
      return res.data;
    },
    onSuccess: () => {
  ...
  const openOptionEditor = (colKey: string) => {
    ...
    api.get("/ui-config/dropdown-options", {
      params: { table: selectedTable, column: colKey, mill_id: user?.millId ?? "default" },
    }).then((res) => {
