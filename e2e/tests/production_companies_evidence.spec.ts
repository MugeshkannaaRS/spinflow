...
test('logged-in /admin/companies renders and captures console/network evidence', async ({ page }) => {
  const evidenceDir = path.resolve('e2e', 'prod-evidence', 'admin_companies');
  ...
  await page.goto('/admin/companies', { waitUntil: 'networkidle', timeout: 120_000 });
  ...
  if (networkRequests.length) {
    fs.writeFileSync(path.join(evidenceDir, 'network-requests.json'), JSON.stringify(networkRequests, null, 2));
  }
